          ...
          <el-table :data="depositBoxList">
            ...
            <el-table-column label="接收地址">
              <template slot-scope="scope">
                <a class="link" :href="'https://etherscan.io/token/' + projData.contractAddr + '?a=' + scope.row.toAddr" target="_blank">{{ getShortStr(scope.row.toAddr, 6) }}</a>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    updateData () {
      ...
      this.$http.post('/api/getProjDepositOrderList', {
        projId: this.$route.params.id
      }).then((res) => {
        if (res.data.errcode === 0) {
