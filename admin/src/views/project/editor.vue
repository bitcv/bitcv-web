  ...
  <div class="media">
    ...
    <el-table :data="perList">
      ...
      <el-table-column label="密码">
        <template slot-scope="scope">{{ scope.row.passwd }}</template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.isSys===0" size="mini" type="primary" @click="authOperate(scope.row.id)">审核通过</el-button>
          <el-button v-if="scope.row.isSys===2" size="mini" type="warning" @click="cancelOperate(scope.row.id)">取消管理员</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    cancelOperate (Id) {
      this.$http.post('/api/cancelOperate', {
        id: Id
      }).then((res) => {
        if (res.data.errcode === 0) {
