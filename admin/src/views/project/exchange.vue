<template>
  <div class="project-main" v-loading="loading">
    <div class="add-box">
      <el-button type="warning" plain @click="visible = true; validate = true">+添加交易所</el-button>
    </div>
    <el-table :data="list" style="width: 100%;">
      ...
      <el-table-column prop="name" label="交易所名称">
      </el-table-column>
    ...
    <el-dialog title="交易所信息" :visible.sync="visible" center @close="handleClose" v-loading="dLoading">
      <el-tabs v-model="activeName">
        <el-tab-pane label="直接选择" name="first">
          <el-form label-width="80px" style="width: 70%; margin: 0 15%;" :model="form1" status-icon :rules="rules1" ref="form1">
            <el-form-item label="机构名称" prop="memberId">
            <el-select v-model="form1.memberId" placeholder="请选择交易所信息">
...
export default {
  ...
  methods: {
    ...mapActions(['getProjExchangeList', 'getExchangeNameList', 'addProjExchange', 'addProjIExchange', 'updProjExchange', 'delProjExchange']),
    fetch () {
      this.loading = true
      this.getProjExchangeList({projId: this.userInfo.userId})
        .then(({dataList = []} = {}) => {
          this.list = dataList
          this.loading = false
        })
    ...
    fetchOption () {
      this.getExchangeNameList()
        .then(({exchangeList = []} = {}) => {
          this.options = exchangeList
        })
    },
    handleEdit (row) {
      this.visible = true
    ...
    handleDel (mId) {
      this.$confirm('删除后无法恢复， 是否继续?', '提示', {
        ...
        this.loading = true
        this.delProjExchange({partnerId: mId})
          .then((data = {}) => {
            this.loading = false
    ...
    handleAdd () {
      ...
      if (this.params.memberId !== undefined && this.params.memberId !== '') { // 直接选择（新增）
        this.addProjExchange({projId: this.params.projId, memberId: this.params.memberId})
          .then(data => {
      ...
      } else { // 有memberId为编辑，无则为手动输入新增
        delete this.params.memberId
        if (this.params.memberId === undefined) delete this.params.memberId
        this[this.params.memberId !== undefined ? 'updProjExchange' : 'addProjIExchange'](this.params)
          .then(data => {
