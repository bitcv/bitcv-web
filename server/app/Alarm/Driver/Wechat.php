 ...
 *          'appsecret'=>'SxyJ3x5DnThcv65p5kcS2CdRatVToZqiJH3fTrnYyYWNrCY9Ew05UroXgG36mPXu', //填写高级调用功能的密钥
 *          'agentid'=>'1', //应用的id
 *          'debug'=>false, //调试开关
...
class Wechat
{
    ...
    public function __construct()
    {
        if (is_null($this->handler)) {
            // 监测access_token
            $access_token = cache('wexin_access_token');
            $weObj        = new WechatBase(config('wechat'));
            if (!$access_token) {
                $access_token = $weObj->checkAuth();
                if (false === $access_token) {
                    exit;
                }
                cache(['wexin_access_token' => $access_token], 119);
            }
            // 保存 Token
    ...
    public function send($data)
    {
        ...
        return $this->handler->sendMessage($data);
    }
}
